...
if __name__ == "__main__":
    ...
    ll = np.random.choice([0, 1], size=(len(tcga)), p=[1./3, 2./3]) 
    ...
    train_tcga = Gene_data(tcga,pd.Series(ll) ,transform=ContrastiveLearningViewGenerator(contrast_transforms))
    tcga_loader = data.DataLoader(train_tcga, batch_size=128, shuffle=True,
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=60, eta_min=1e-4, last_epoch=-1)
    ...
    #     contrast_transforms = transforms.RandomChoice([normal_mixup(train_normal, 0.5  ), Gaussian(0.05)])
    ...
    # train_data = Gene_data(tcga,pd.Series(ll) ,transform=None)
    train_loader = data.DataLoader(train_data, batch_size=batch_size, shuffle=True,
